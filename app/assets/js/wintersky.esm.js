import { Object3D, Texture, NearestFilter, Vector3, Euler, Plane, PlaneGeometry, Mesh, BufferAttribute, Quaternion, Line3, Sphere, ShaderMaterial, Vector2, FrontSide, DoubleSide, AdditiveBlending, NormalBlending, CubicBezierCurve, SplineCurve } from 'three';
...
class Config {
	constructor(scene, config, options = 0) {
		this.scene = scene;
		this.texture = new Texture(new Image());
...
const collisionPlane = new Plane().setComponents(0, 1, 0, 0);
...
class Particle {
	constructor(emitter) {
		...
		this.facing_direction = new Vector3();
	...
	tick(jump) {
		...
		if (this.emitter.config.particle_motion_mode === 'dynamic') {
			...
			if (this.emitter.config.particle_lifetime_kill_plane.find(v => v)) {
				...
				var start_point = new Vector3().copy(this.position).addScaledVector(this.speed, -step);
				var end_point = new Vector3().copy(this.position);
				...
				var line = new Line3(start_point, end_point);
			...
			if (
				...
				let sphere = new Sphere(this.position, radius);
				let previous_pos = new Vector3().copy(this.position).addScaledVector(this.speed, -step);
				let line = new Line3(previous_pos, this.position);
...
function createCurveSpline(curve) {
	switch (curve.mode) {
		...
		case 'bezier':
			var vectors = [];
			curve.nodes.forEach((val, i) => {
				vectors.push(new Vector2(i/3, val));
			});
			return new CubicBezierCurve(...vectors);
	}
}
function calculateCurve(emitter, curve, curve_key, params) {
	...
	if (curve.mode == 'linear') {
		...
		var vectors = [
			new Vector2(before.time + time_diff * (0/3), before.right_value),
			new Vector2(before.time + time_diff * (1/3), before.right_value + before.right_slope * (1/3)),
			new Vector2(before.time + time_diff * (2/3), after.left_value - after.left_slope * (1/3)),
			new Vector2(before.time + time_diff * (3/3), after.left_value),
		];
		var spline = new CubicBezierCurve(...vectors);
...
class Emitter extends EventClass {
	...
	updateMaterial() {
		...
		this.material.side = (material === 'particles_alpha' || material === 'particles_opaque') ? FrontSide : DoubleSide;
		this.material.blending = material === 'particles_add' ? AdditiveBlending : NormalBlending;
	}
				...
				if (config) {
					...
					if (this.local_space.parent) {
						if (!this.config.space_local_position) {
							let offset = this.local_space.getWorldPosition(new Vector3());
							position.add(offset);
						}
					}
